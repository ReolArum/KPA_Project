...
public class ExplorationState
{
    ...
    public int   remainingEnemyTickets; // [MOD] 남은 적 전용 선택권 개수 (-1이면 무제한)
    ...
    // 획득한 특수 오브젝트/환경 오브젝트 ID 리스트
    public List<string> foundEnvObjectIds = new List<string>();
    ...
    public void Reset(float startTime, int maxEnemyTickets, Vector3 startPos)
    {
        remainingTime         = startTime;
        remainingEnemyTickets = maxEnemyTickets;
        collectedGold         = 0;
        currentPosition       = startPos; // [ADD] 시작 위치 초기화
        foundEnvObjectIds.Clear();
